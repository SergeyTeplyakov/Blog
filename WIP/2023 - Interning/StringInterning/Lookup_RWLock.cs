...
internal class Lookup_RWLock<TKey, TValue> : ILookup<TKey, TValue>
    where TKey : class
{
    ///// <summary>
    ...
    {
        var (valueSet, newlyCreated) = GetOrCreateGrouping(key, value);

        if (newlyCreated)
        {
            // THe new grouping instance was created. No need to add another value!
            return true;
        }
    ...
    private (Grouping grouping, bool newlyCreated) GetOrCreateGrouping(TKey key, TValue value)
    {
        ...
        try
        {
            if (!content.TryGetValue(key, out var valueSet))
            {
                try
                {
                    ...
                    {
                        var grouping = new Grouping(key, comparer, value);
                        content.Add(key, grouping);
                        return (grouping, newlyCreated: true);
                    }
                }
                finally
                {
                ...
                return (content[key], newlyCreated: false);
            }

            return (valueSet, newlyCreated: false);
        }
        finally
        {
